...
import {MultiAdminSingleHolderAccessControl} from "../src/utils/MultiAdminSingleHolderAccessControl.sol";
...
contract TestMultiAdminSingleHolderAccessControl is MultiAdminSingleHolderAccessControl {
    constructor(address initialAdmin, bytes32 adminRole) {
...
contract MultiAdminSingleHolderAccessControlNonUpgradeableTest is Test {
    ...
    function setUp() public {
        ...
        accessControl = new TestMultiAdminSingleHolderAccessControl(admin, DEFAULT_ADMIN_ROLE);
    ...
    function test_ConstructorWithDifferentParams() public {
        ...
        TestMultiAdminSingleHolderAccessControl newAccessControl =
            new TestMultiAdminSingleHolderAccessControl(newAdmin, newRole);
    ...
    // Test constructor state consistency
    function test_ConstructorStateConsistency() public view {
        // Verify that after constructor, the contract is in expected state
        assertTrue(accessControl.hasRole(DEFAULT_ADMIN_ROLE, admin));
        assertTrue(accessControl.isRoleAdmin(DEFAULT_ADMIN_ROLE, DEFAULT_ADMIN_ROLE));
    ...
    // Test edge case: constructor with zero address
    function test_ConstructorWithZeroAddress() public {
        TestMultiAdminSingleHolderAccessControl newAccessControl =
            new TestMultiAdminSingleHolderAccessControl(address(0), DEFAULT_ADMIN_ROLE);

        // Should work - address(0) can have roles
        assertTrue(newAccessControl.hasRole(DEFAULT_ADMIN_ROLE, address(0)));
    }

    // Test edge case: constructor with zero role
    function test_ConstructorWithZeroRole() public {
        TestMultiAdminSingleHolderAccessControl newAccessControl =
            new TestMultiAdminSingleHolderAccessControl(admin, bytes32(0));
    ...
    // Test multiple roles for single address and role transfer
    function test_MultipleRolesForSingleAddress_WithRoleTransfer() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                IMultiAdminSingleHolderAccessControl.AccessControlUnauthorizedAccount.selector, user1, OPERATOR_ROLE
            )
        );
        vm.prank(user1);
        ...
        assertEq(result2, "Custom role access granted");
    }
}
