...
// Mock contract for UniswapV2Router with fixed price swaps
contract UniswapV2Mock is IUniswapV2Router02 {
    ...
    function swapExactTokensForTokens(
        ...
        require(path.length >= 2, "UniswapV2Mock: INVALID_PATH");
        ...
        for (uint256 i = 0; i < path.length - 1; i++) {
            amounts[i + 1] = (amounts[i] * prices[path[i]][path[i + 1]]) / 1e18;
        }
        ...
        returns (uint256[] memory)
    {
        ...
        amounts[0] = amountIn;
        amounts[1] = (amountIn * prices[path[0]][path[1]]) / 1e18;
        return amounts;
    }
        ...
        returns (uint256[] memory)
    {
        uint256[] memory amounts = new uint256[](path.length);
        amounts[1] = amountOut;
        amounts[0] = (amountOut * 1e18) / prices[path[0]][path[1]];
        return amounts;
    }
    ...
    // Return zero address for factory
    function factory() external pure override returns (address) {
        return address(0);
    }

    // Return zero address for WETH
